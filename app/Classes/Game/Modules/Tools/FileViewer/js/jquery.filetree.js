...
//
// (Modified by: Kasper F.M - https://kasperfm.com)
...
(function( $ ) {
    ...
    openfolder = function(dir) {
        ...
        ret = $.ajax({
            type: 'POST',
            url : options.script,
            data : {dir : dir,_token: window.Laravel.csrfToken},
            context : $this,
            dataType: 'json',
            ...
            for(ij=0; ij<datas.length; ij++){
                ...
                ret += '<a href="#" rel="'+datas[ij].fileid+'" data-file="'+dir+datas[ij].file+'/" data-type="'+datas[ij].type+'">'+datas[ij].file+'</a>';
                ret += '</li>';
            }
            ret += '</ul>';
    ...
    setevents = function(){
        ...
        $this.find('li a').bind('click', function() {
            options.onclick(this, $(this).attr('data-type'),$(this).attr('data-file'),$(this).attr('rel'));
