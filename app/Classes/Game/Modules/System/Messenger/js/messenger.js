...
$(document).ready(function() {
    ...
    socket.on('new message', function(data){
        $chat.append('<li class="tooltip"><span class="tooltiptext">'+data.timestamp+'</span><span class="msg"><b>' + data.nick + ': </b>' + $('<span>'+data.msg+'</span>').text() + '</span></li>');
        $("#chatBox").animate({ scrollTop: $('#chatBox')[0].scrollHeight}, 1000);
    });

    socket.on('whisper', function(data){
        $chat.append('<li class="tooltip"><span class="tooltiptext">'+data.timestamp+'</span><span class="whisper"><b>' + data.nick + ': </b>' + $('<span>'+data.msg+'</span>').text() + '</span></li>');
        $("#chatBox").animate({ scrollTop: $('#chatBox')[0].scrollHeight}, 1000);
    });
